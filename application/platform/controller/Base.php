...
class Base extends Controller
{
    ...
    public function initialize()
    {
        ...
        wxpayNotify();
        $admin = getPlatformAdmin();
        if (empty($admin)) {
            $this->redirect(url('platform/Login/index'));
        }
        $adminInfo = Db::name('platform')->where('id', $admin['id'])->field('password')->find();
        ...
        $this->padmin_id = $admin['id'];
        $node = MODULE_NAME . '/' . CONTROLLER_NAME . '/' . ACTION_NAME;
        $t_menu = Db::name('platform_menu')->where('pid', 0)->order('sort ASC')->select();
        $allMenu = Db::name('platform_menu')->order('sort ASC')->select();
        $nowMenu = Db::name('platform_menu')->where('url', $node)->order('sort ASC')->find();
        $topNode = null;
        $menu2 = null;
        $menu_title = '';
        ...
        $this->assign('platform_menu', $menu2);
        $this->assign('controller_name', CONTROLLER_NAME);
        $this->assign('action_name', ACTION_NAME);
        $this->assign('padmin', $admin);
    ...
    {
        $menu = Db::name('platform_menu')->where(['pid' => 0, 'url' => $node])->order('sort ASC')->find();
